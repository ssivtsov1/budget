...
<script>
    ...
    window.addEventListener('load', function(){
        ...
        localStorage.setItem("gr_n_cnt", 'false');
        localStorage.setItem("gr_cagent", 'false');
    });
    ...
    function def_gr(v,name){
        ...
        if(localStorage.getItem("gr_name_service")=='true'){
            p1=$('#analytics_ost-ord').val();
            if (p1.indexOf("gr_name_service") == -1)
                $('#analytics_ost-ord').val(p1+" gr_name_service");
        }

        if(localStorage.getItem("gr_type_act")=='true'){
            p2=$('#analytics_ost-ord').val();
            if (p2.indexOf("gr_type_act") == -1)
                $('#analytics_ost-ord').val(p2+" gr_type_act");
        }

        if(localStorage.getItem("gr_type_tmc")=='true'){
            p3=$('#analytics_ost-ord').val();
            if (p3.indexOf("gr_type_tmc") == -1)
                $('#analytics_ost-ord').val(p3+" gr_type_tmc");
        }
        if(localStorage.getItem("gr_type_repair")=='true'){
            p4=$('#analytics_ost-ord').val();
            if (p4.indexOf("gr_type_repair") == -1)
                $('#analytics_ost-ord').val(p4+" gr_type_repair");
        }

        if(localStorage.getItem("gr_page_b")=='true'){
            p5=$('#analytics_ost-ord').val();
            if (p5.indexOf("gr_page_b") == -1) {
                $('#analytics_ost-ord').val(p5+" gr_page_b");
            }
        }
        if(localStorage.getItem("gr_spec")=='true'){
            p6=$('#analytics_ost-ord').val();
            if (p6.indexOf("gr_spec") == -1)
                $('#analytics_ost-ord').val(p6+" gr_spec");
        }
        if(localStorage.getItem("gr_obj")=='true'){
            p6=$('#analytics_ost-ord').val();
            if (p6.indexOf("gr_obj") == -1)
                $('#analytics_ost-ord').val(p6+" gr_obj");
            //alert($('#analytics-ord').val());
        }
        if(localStorage.getItem("gr_name_tmc")=='true'){
            p6=$('#analytics_ost-ord').val();
            if (p6.indexOf("gr_name_tmc") == -1)
                $('#analytics_ost-ord').val(p6+" gr_name_tmc");
        }
        if(localStorage.getItem("gr_edizm")=='true'){
            p6=$('#analytics_ost-ord').val();
            if (p6.indexOf("gr_edizm") == -1)
                $('#analytics_ost-ord').val(p6+" gr_edizm");
        }
        if(localStorage.getItem("gr_n_cnt")=='true'){
            p6=$('#analytics_ost-ord').val();
            if (p6.indexOf("gr_n_cnt") == -1)
                $('#analytics_ost-ord').val(p6+" gr_n_cnt");
        }
        if(localStorage.getItem("gr_cagent")=='true'){
            p6=$('#analytics_ost-ord').val();
            if (p6.indexOf("gr_cagent") == -1)
                $('#analytics_ost-ord').val(p6+" gr_cagent");
        }
    ...
    function sel_all(v){
        ...
        if(v) {
            ...
            $('#analytics_ost-gra_nakop').prop('checked', true);
            $('#analytics_ost-gra_nakop_z').prop('checked', true);
            $('#analytics_ost-gra_nakop_y').prop('checked', true);
            $('#analytics_ost-gra_nakop_v').prop('checked', true);
            $('#analytics_ost-gra_nakop_q').prop('checked', true);

        }
        else {
            $('#analytics_ost-gra_nakop_z').prop('checked', false);
            $('#analytics_ost-gra_nakop').prop('checked', false);
            $('#analytics_ost-gra_nakop_y').prop('checked', false);
            $('#analytics_ost-gra_nakop_v').prop('checked', false);
            $('#analytics_ost-gra_nakop_q').prop('checked', false);

        }
    }

    function proc_ok(){
        if($('#analytics_ost-gra_oper').val()==5) return;
        if(localStorage.getItem("gr_name_service")=='true' ||
            ...
            localStorage.getItem("gr_cagent")=='true' ||
            localStorage.getItem("gr_n_cnt")=='true')
    {

        if(!($('#analytics_ost-gra_nakop_q').prop("checked") ||
            $('#analytics_ost-gra_nakop').prop("checked") ||
            $('#analytics_ost-gra_nakop_y').prop("checked") ||
            $('#analytics_ost-gra_nakop_v').prop("checked") ||
            $('#analytics_ost-gra_nakop_z').prop("checked")))
        {
            alert('Введіть поле групування!');
            $('#analytics_ost-ord').val("error");
        }
    }
...
<!--            -->
            ...
            <div class="pole-gr">
                ...
                <?= $form->field($model, 'gr_n_cnt')->checkbox(['onChange' => 'def_gr($(this).prop("checked"),"#analytics-gr_n_cnt")']); ?>
                <?= $form->field($model, 'gr_cagent')->checkbox(['onChange' => 'def_gr($(this).prop("checked"),"#analytics-gr_cagent")']); ?>
            ...
            <div class="pole-gra">
                ...
                <?= $form->field($model, 'gra_nakop_v')->checkbox(); ?>
                <?= $form->field($model, 'gra_nakop')->checkbox(); ?>
                <?= $form->field($model, 'gra_nakop_y')->checkbox(); ?>
