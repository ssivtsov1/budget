...
//            ]) ?>
            ...
            <div class="clearfix"></div>
            <?= $form->field($model, 'znakop_y')->
            dropDownList([1 => '=',2 => '>',3 => '>=',4 => '<',5 => '<=',6 => '<>',7 => 'пусто',8 => 'не пусто'],['prompt'=>'']) ?>
            <?=$form->field($model, 'nakop_y')->
            textInput() ?>
            <div class="clearfix"></div>
...
function show_chrep(p){
    if (p == 1) {
    $('.ch_repair').show();
    $('.field-inputdataost-type_repair').hide();
    $('label[for=inputdataost-else_repair]').css('font-weight', 'bold');
    }
    else {
    $('.ch_repair').hide();
    $('.field-inputdataost-type_repair').show();
    $('label[for=inputdataost-else_repair]').css('font-weight', 'normal');
}
}
...
function show_chservice(p){
    if (p == 1) {
        $('.ch_service').show();
        $('.field-inputdataost-name_service').hide();
        $('label[for=inputdataost-else_service]').css('font-weight', 'bold');
    }
    else {
       $('.ch_service').hide();
        $('.field-inputdataost-name_service').show();
        $('label[for=inputdataost-else_service]').css('font-weight', 'normal');
    }
}
...
function show_chspec(p){
    if (p == 1) {
        $('.ch_spec').show();
        $('.field-inputdataost-spec').hide();
        $('label[for=inputdataost-else_spec]').css('font-weight', 'bold');
    }
    else {
        $('.ch_spec').hide();
        $('.field-inputdataost-spec').show();
        $('label[for=inputdataost-else_spec]').css('font-weight', 'normal');
    }
}
</script>
