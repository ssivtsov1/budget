...
// Справочник услуг
    ...
    // Справочник моих проектов
    public function actionSprav_project()
    {
        $searchModel = new Projects();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        return $this->render('sprav_project', ['model' => $searchModel,
...
//    ~ Удаление записей из справочника
    ...
    {   // $id  id записи
        ...
        if($mod=='sprproject')
        return $this->redirect(['sprav/sprav_project']);
    }
...
//    ~ Обновление записей из справочника
    public function actionUpdate($id,$mod)
    {
        ...
        if ($model->load(Yii::$app->request->post()))
        {
            ...
            if($mod=='sprproject')
            return $this->render('update_sprproject', [
                'model' => $model,
            ]);
        }
    }
//    ~ Срабатывает при нажатии кнопки добавления проекта
